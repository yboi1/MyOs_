...
void intr_keyboard_handler(void) {
    ...
    if(scancode == 0xe0){   // 判断是否为产生的多个扫描码, 结束此次中断, 等待下一个扫描码
        ext_scancode = true;
        return;
    }

    if (ext_scancode) {
        scancode = ((0xe000) | scancode);   // 如果上次是0xe0 开头的将扫描码合并
        ext_scancode = false;
    }

    break_code = ((scancode & 0x0080) != 0);    // 获取 break_code 判断是否为断码

    if(break_code) {    // 判断是否是按键弹起产生的扫描码
